...
pub struct BeaconClient(pub(crate) SyncCommitteeProver);

impl BeaconClient {
    pub fn new(node_url: impl AsRef<str>) -> BeaconClient {
        (SyncCommitteeProver::new(node_url.as_ref().to_string()))
    }

    pub async fn fetch_step_args<S: Spec>(&self) -> anyhow::Result<SyncStepArgs<Test>> {    
...
async fn fetch_step_args() {
    ...
    let args = client.fetch_step_args::<Test>().await.unwrap();
